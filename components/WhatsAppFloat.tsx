...
export default function WhatsAppFloat() {
  ...
  return (
    <>
      {visible && (
        // Floats on all viewports (mobile + desktop) again — the mobile
        // sticky 'Book Now' bar that previously claimed the bottom on
        // phones was removed because it obstructed page content.
        <div className="fixed right-6 bottom-6 z-50">
          {/* La entrada (aparecer girando) y el hover/tap ahora son CSS:
              `wa-in` para el montaje y clases Tailwind para escalar. Antes
              esto arrastraba framer-motion al bundle inicial del home. */}
          <button
            onClick={openWhatsApp}
            className="wa-in relative w-16 h-16 rounded-full bg-green-600 hover:bg-green-700 shadow-2xl shadow-green-600/50 flex items-center justify-center transition-all hover:scale-110 active:scale-95 group"
